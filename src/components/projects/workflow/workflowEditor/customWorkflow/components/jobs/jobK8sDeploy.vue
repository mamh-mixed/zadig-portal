<template>
  <div class="job-k8s-deploy">
    <el-form :label-width="formLabelWidth" :model="job" ref="ruleForm"  label-position="left" class="mg-t24 mg-b24">
      ...
      <el-form-item class="status-check" :label="$t(`workflow.dockerRegistry`)" prop="spec.docker_registry_id" :rules="{required: true, message: '镜像仓库不能为空', trigger: ['blur','change']}">
      ...
      <el-form-item class="status-check" :label="$t(`workflow.containerName`)" prop="spec.targets"  :required="job.spec.source && job.spec.source !== 'runtime'" >
