  ...
  <div class="service-details-container">
    ...
    <div class="info-card" v-if="envSource ==='' || envSource === 'spock'">
      ...
      <div class="info-body fundamental-ops">
        <template>
          <router-link v-if="checkPermissionSyncMixin({projectName: projectName, action: 'manage_environment'})"
          ...
          <el-button v-if="checkPermissionSyncMixin({projectName: projectName, action: 'manage_environment'})" @click="showExport"
        ...
        <template>
          <el-table :data="currentService.scales"
              ...
              <template slot-scope="scope">
                <div v-for="(item,index) of scope.row.images"
                  ...
                  <template v-if="!item.edit">
                    ...
                    <el-button v-hasPermi="{projectName: projectName, action: 'manage_environment',isBtn:true}" @click="showEditImage(item)"
                               type="primary"
                               plain
                               size="mini"
                               class="edit-btn"
            ...
            <el-table-column props="replicas"
                             ...
                             label="副本数量">
              <template slot-scope="scope">
                <el-input-number v-if="checkPermissionSyncMixin({projectName: projectName, action: 'manage_environment'})" size="mini"
                                 :min="0"
            ...
            <el-table-column  label="操作"
                             width="220px">
              <template slot-scope="scope">
                <el-button v-hasPermi="{projectName: projectName, action: 'manage_environment',isBtn:true}" @click="restartService(scope.row.name,scope.row.type)"
                           size="mini">重启实例</el-button>
                <el-button v-hasPermi="{projectName: projectName, action: 'manage_environment',isBtn:true}" @click="showScaleEvents(scope.row.name,scope.row.type)"
                  ...
                  <transition name="fade">
                    <div v-if="activePod[scope.$index].name"
                      ...
                      <el-row v-for="container of activePod[scope.$index].containers"
                        ...
                        <el-col :span="5"
                                class="op-buttons">
                          <el-button v-hasPermi="{projectName: projectName, action: 'debug_pod',isBtn:true}" @click="showContainerExec(activePod[scope.$index].name,container.name)"
                                     :disabled="!activePod[scope.$index].canOperate"
                                     icon="iconfont iconTerminal"
                                     size="small"> 调试</el-button>
                          <el-button v-hasPermi="{projectName: projectName, action: 'manage_environment',isBtn:true}" @click="showContainerLog(activePod[scope.$index].name,container.name)"
                                     :disabled="!activePod[scope.$index].canOperate"
