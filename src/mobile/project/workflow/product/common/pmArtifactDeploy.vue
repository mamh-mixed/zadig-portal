<template>
  <div>
    ...
    <el-form-item label="对象存储">
      <el-select v-model="pickedStorage" filterable clearable @change="changeStorage" size="medium" class="full-width">
        <el-option v-for="(item,index) of objectstorageList" :key="index" :label="`${item.bucket}(${item.endpoint})`" :value="item.id"></el-option>
      </el-select>
    </el-form-item>
            ...
            <el-row class="build-row">
              <template>
                <el-col :span="24">
                  ...
                  <el-select
                    v-if="tarFileMap[scope.row.name] && tarFileMap[scope.row.name].length > 0"
                    ...
                    placeholder="请选择交付物"
                  >
                    <el-option v-for="(source,index) in tarFileMap[scope.row.name]" :key="index" :label="source.file_name" :value="source"></el-option>
                  </el-select>
                  <el-tooltip v-else content="交付物不存在，请手动输入交付物信息" placement="top" popper-class="gray-popper">
                    <el-input v-model="scope.row.file_name" class="short-input" size="small" style="width: 100%;" placeholder="请填写交付物信息"></el-input>
                  </el-tooltip>
                </el-col>
              </template>
            </el-row>
  ...
  methods: {
    ...
    getArtifactFiles (val) {
      return new Promise((resolve, reject) => {
        ...
        getArtifactFileAPI(payload, objectStorageId, projectName).then(
          files => {
            files = files || []
    ...
    changeStorage (val) {
      if (val) {
        this.tarFileMap = []
        const allClickableServeiceNames = this.pickedTargetServices.map(
    ...
    getStorageListAPI(key).then(res => {
      ...
      if (this.forcedUserInput) {
        ...
        this.getArtifactFiles(
          this.forcedUserInput.artifact_args.map(r => r.name)
        )
        return
      }
      if (res && res.length > 0) {
